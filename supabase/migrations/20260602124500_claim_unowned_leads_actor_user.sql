-- Server actions use a service-role Supabase client for writes, so auth.uid()
-- is not populated inside Postgres. Accept an explicit actor user id from the
-- already-authorized server action while preventing browser-authenticated callers
-- from spoofing a different user id.

drop function if exists public.claim_unowned_lead(uuid, uuid);
...
begin
  ...
  if v_actor_user_id is null then
    raise exception 'Actor user is required to claim a lead' using errcode = '22023';
  end if;

  if not public.is_org_owner_or_admin(p_organization_id, v_actor_user_id) then
  ...
  if v_lead.organization_id is not null then
    if not public.is_org_owner_or_admin(v_lead.organization_id, v_actor_user_id) then
...
revoke all on function public.claim_unowned_lead(uuid, uuid, uuid) from public;
grant execute on function public.claim_unowned_lead(uuid, uuid, uuid) to authenticated;
